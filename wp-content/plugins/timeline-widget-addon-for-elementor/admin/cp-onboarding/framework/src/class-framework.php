...
final class Framework {
	...
	private function script_data() {
		...
		$data = array(
			...
			'labels'  => array(
				'loading'     => __( 'Please wait…', 'timeline-widget-addon-for-elementor' ),
				'redirecting' => __( 'Redirecting…', 'timeline-widget-addon-for-elementor' ),
			),
		);
	...
	public function render_page() {
		if ( ! current_user_can( $this->config->capability() ) ) {
			wp_die( esc_html__( 'You do not have permission to access this page.', 'timeline-widget-addon-for-elementor' ) );
		}
	...
	public function ajax_prepare() {
		...
		if ( ! $method ) {
			wp_send_json_error( array( 'message' => __( 'Invalid selection.', 'timeline-widget-addon-for-elementor' ) ), 400 );
		}
		...
		if ( empty( $method['demo'] ) ) {
			...
			if ( '' === $url ) {
				wp_send_json_error( array( 'message' => __( 'No destination configured.', 'timeline-widget-addon-for-elementor' ) ), 400 );
			}
		...
		if ( empty( $demo_config ) ) {
			wp_send_json_error( array( 'message' => __( 'Demo generator is unavailable.', 'timeline-widget-addon-for-elementor' ) ), 500 );
		}
	...
	public function ajax_install() {
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error(
				array( 'errorMessage' => __( 'Sorry, you are not allowed to install plugins on this site.', 'timeline-widget-addon-for-elementor' ) ),
				403
			);
		}
		...
		if ( '' === $slug ) {
			wp_send_json_error( array( 'errorMessage' => __( 'No plugin specified.', 'timeline-widget-addon-for-elementor' ) ), 400 );
		}

		if ( ! in_array( $slug, $this->installable_slugs(), true ) ) {
			wp_send_json_error( array( 'errorMessage' => __( 'This plugin cannot be installed from here.', 'timeline-widget-addon-for-elementor' ) ), 403 );
		}
		...
		if ( null === $result ) {
			wp_send_json_error(
				array( 'errorMessage' => __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'timeline-widget-addon-for-elementor' ) ),
				500
			);
		}
