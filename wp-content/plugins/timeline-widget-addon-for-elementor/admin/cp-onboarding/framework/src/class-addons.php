...
// phpcs:disable WordPress.WP.I18n.TextDomainMismatch -- Framework strings use the plugin text domain from config at runtime.
/**
...
final class Addons {
	...
	public static function resolve( array $addons, Config $config ) {
		...
		foreach ( $addons as $addon ) {
			...
			$label = 'pro' === $type
				? ( isset( $addon['upgrade_label'] ) ? $addon['upgrade_label'] : __( 'Upgrade to Pro', 'timeline-widget-addon-for-elementor' ) )
				: $state['label'];
	...
	private static function state_for( $slug ) {
		...
		if ( $is_active ) {
			return array( 'state' => 'activated', 'label' => __( 'Activated', 'timeline-widget-addon-for-elementor' ) );
		}
		if ( $installed ) {
			return array( 'state' => 'activate', 'label' => __( 'Activate', 'timeline-widget-addon-for-elementor' ) );
		}
		return array( 'state' => 'install', 'label' => __( 'Install Free', 'timeline-widget-addon-for-elementor' ) );
	}
