...
/**
 * Pick the addon card for a method group: free install when not activated, otherwise Pro upsell.
 *
...
$cpo_addon_actions = function ( $cpo_addon ) {
	...
	<?php if ( ! empty( $cpo_addon['type'] ) && 'pro' === $cpo_addon['type'] ) : ?>
		...
		<?php if ( ! empty( $cpo_addon['learn_more'] ) ) : ?>
			<a class="cpo-button cpo-button-secondary" href="<?php echo esc_url( $cpo_addon['learn_more'] ); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__( 'Explore Layouts', 'timeline-widget-addon-for-elementor' ); ?>								<span class="dashicons dashicons-arrow-right-alt2 cpo-button-chevron" aria-hidden="true"></span></a>
	...
	<?php if ( 'activated' === $cpo_addon['state'] && ! empty( $cpo_addon['setup_url'] ) ) : ?>
		<a class="cpo-button cpo-button-secondary cpo-setup-guide" href="<?php echo esc_url( $cpo_addon['setup_url'] ); ?>"><?php echo esc_html__( 'Check Setup Guide', 'timeline-widget-addon-for-elementor' ); ?></a>
	<?php endif; ?>
	<?php if ( ! empty( $cpo_addon['learn_more'] ) ) : ?>
		<a class="cpo-button cpo-button-secondary" href="<?php echo esc_url( $cpo_addon['learn_more'] ); ?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html__( 'Learn More', 'timeline-widget-addon-for-elementor' ); ?></a>
	<?php endif; ?>
	<?php
};
					...
					<button type="button"
						...
						<div class="cpo-method-top">
							...
							<?php if ( ! empty( $cpo_method['_locked'] ) ) : ?>
								<span class="cpo-badge cpo-badge-pro"><?php echo esc_html__( 'Pro', 'timeline-widget-addon-for-elementor' ); ?></span>
						...
						<?php if ( ! empty( $cpo_method['best_for'] ) ) : ?>
							<p class="cpo-best-for"><strong><?php echo esc_html__( 'Best for', 'timeline-widget-addon-for-elementor' ); ?>:</strong> <?php echo esc_html( $cpo_method['best_for'] ); ?></p>
						<?php endif; ?>
					</button>
				<?php endforeach; ?>
			</div>
				...
				<div class="cpo-content-grid">
					...
					<div class="cpo-video">
						<?php if ( ! empty( $cpo_method['video']['title'] ) ) : ?>
							<h3><?php echo esc_html__( 'Watch how it works', 'timeline-widget-addon-for-elementor' ); ?></h3>
						<?php endif; ?>
						<button type="button"
							class="cpo-video-box"
							data-type="<?php echo esc_attr( $cpo_method['type'] ); ?>"
					...
					<div class="cpo-guide">
						<?php if ( ! empty( $cpo_method['steps'] ) ) : ?>
							<div class="cpo-guide-heading">
								<h3><?php echo esc_html__( 'Quick Setup Guide', 'timeline-widget-addon-for-elementor' ); ?></h3>
						...
						<div class="cpo-cta-bar">
							...
							if ( ! empty( $cpo_method['_locked'] ) && ! empty( $cpo_method['upgrade_url'] ) ) :
								...
								<a class="button button-primary cpo-button-large cpo-upgrade"
									...
									<?php echo esc_html( ! empty( $cpo_method['upgrade_label'] ) ? $cpo_method['upgrade_label'] : __( 'Unlock in Pro →', 'timeline-widget-addon-for-elementor' ) ); ?>
								</a>
								<?php
