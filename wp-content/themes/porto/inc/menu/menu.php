...
function porto_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
	$check = array( 'icon_type', 'icon', 'icon_svg', 'icon_img', 'nolink', 'hide', 'mobile_hide', 'popup_mobile_hide', 'cols', 'popup_type', 'popup_pos', 'popup_cols', 'popup_max_width', 'popup_bg_image', 'popup_bg_pos', 'popup_bg_repeat', 'popup_bg_size', 'popup_style', 'block', 'tip_label', 'tip_color', 'tip_bg', 'preview', 'preview_fixed' );
	...
	<div class="menu-dependency-wrap">
		<p class="description description-wide">
			<label for="edit-menu-item-icon_type-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Icon Type', 'porto' ); ?><br />
			...
			if ( '' == $item->hide || '' == $item->mobile_hide ) {
				...
				if ( isset( $item->icon_type ) && 'svg' == $item->icon_type ) {
					...
					if ( ! empty( $item->icon_img ) ) {
						$item_output .= wp_get_attachment_image( (int) $item->icon_img, 'blog-masonry-small' );
					}
					$item_output .= $args->link_before;
					$item_output .= apply_filters( 'the_title', $item->title, $item->ID );
				} else {
